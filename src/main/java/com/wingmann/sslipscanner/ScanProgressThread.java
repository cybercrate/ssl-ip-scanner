...
public class ScanProgressThread extends Thread {
    private final List<ThreadedAddrScan> threads;
    private final int overallCount;
    private int finishedCount;

    public ScanProgressThread(List<ThreadedAddrScan> threads, int overallCount) {
    ...
    @Override
    public void run() {
        for (ThreadedAddrScan thread : threads) {
            thread.start();
        }
        ...
        System.out.printf("%s%nFinished%n", domains);
    }

    private void writeToFile(List<String> domains) {
        try (FileWriter writer = new FileWriter("domain-list.txt", false)) {
